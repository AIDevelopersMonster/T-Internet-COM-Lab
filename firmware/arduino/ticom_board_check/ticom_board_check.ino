...
void printWifiStatus() {
  ...
  if (mode != WIFI_MODE_NULL) Serial.printf("MAC: %s\n", WiFi.macAddress().c_str());
...
void scanWifiNetworks() {
  ...
  for (int16_t index = 0; index < count; ++index) {
    ...
    Serial.printf("%2d. %s | RSSI %d dBm | CH %d | %s\n",
                  ...
                  WiFi.channel(index), openNetwork ? "open" : "secured");
  }
  WiFi.scanDelete();
}
...
void connectWifi(const String &arguments) {
  ...
  while (WiFi.status() != WL_CONNECTED && millis() - startedAt < board::WIFI_CONNECT_TIMEOUT_MS) {
    Serial.print('.');
    delay(500);
  }
  Serial.println();
...
void testSdReadWrite() {
  ...
  if (written != payload.length()) {
    Serial.printf("SD TEST: FAIL, wrote %u of %u bytes.\n",
                  static_cast<unsigned>(written), static_cast<unsigned>(payload.length()));
    SD.remove(board::SD_TEST_PATH);
    return;
  }
...
void handleCommand(String command) {
  ...
  if (normalized == "HELP" || normalized == "?") printHelp();
  else if (normalized == "STATUS") printStatus();
  else if (normalized == "INFO" || normalized == "SYSTEM") printSystemInfo();
  else if (normalized == "ETH") printEthernetStatus();
  else if (normalized == "WIFI STATUS" || normalized == "WIFI") printWifiStatus();
  else if (normalized == "WIFI SCAN") scanWifiNetworks();
  else if (normalized.startsWith("WIFI CONNECT ")) connectWifi(command.substring(13));
  else if (normalized == "WIFI OFF") disableWifi();
  else if (normalized == "SD") printSdStatus();
  else if (normalized == "SD LIST") listRootDirectory();
  else if (normalized == "SD TEST") testSdReadWrite();
  else if (normalized == "LED RED") { setRgb(255, 0, 0); Serial.println(F("RGB: red")); }
  else if (normalized == "LED GREEN") { setRgb(0, 255, 0); Serial.println(F("RGB: green")); }
  else if (normalized == "LED BLUE") { setRgb(0, 0, 255); Serial.println(F("RGB: blue")); }
  else if (normalized == "LED WHITE") { setRgb(255, 255, 255); Serial.println(F("RGB: white")); }
  else if (normalized == "LED OFF") { setRgb(0, 0, 0); Serial.println(F("RGB: off")); }
...
void readSerialCommands() {
  while (Serial.available() > 0) {
    ...
    if (inputLine.length() < board::SERIAL_COMMAND_MAX_LENGTH) inputLine += value;
    else {
      inputLine = "";
...
void initSd() {
  ...
  if (sdMounted) Serial.printf("microSD mounted: %llu MB\n", SD.cardSize() / (1024ULL * 1024ULL));
  else Serial.println(F("microSD not detected. This is acceptable if no card is installed."));
}
...
void initEthernet() {
  ...
  resetEthernetPhy();
  const bool beginResult = ETH.begin(board::ETH_PHY_TYPE, board::ETH_PHY_ADDR,
                                     board::ETH_MDC_PIN, board::ETH_MDIO_PIN,
                                     board::ETH_POWER_PIN, board::ETH_CLK_MODE);
  if (!beginResult) {
...
void setup() {
  ...
  Serial.println(F("Stage 3: system diagnostics, Ethernet, microSD, Wi-Fi and RGB. Modem disabled."));
  initSd();
  initEthernet();
  printHelp();
  Serial.print(F("> "));
